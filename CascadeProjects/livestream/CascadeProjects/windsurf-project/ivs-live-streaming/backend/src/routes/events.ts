...
router.get('/:id/access', requireAuth, async (req, res) => {
  ...
  const seasonTicket = await pool.query(
    'SELECT status, created_at FROM season_tickets WHERE user_id = $1',
    [userId]
  );
  ...
  const seasonTicketPurchasedAt = seasonTicket.rows[0]?.created_at as string | undefined;
...
router.get('/user/ticket-status', requireAuth, async (req, res) => {
  ...
  try {
    ...
    return res.json({
      tickets,
      hasSeasonTicket,
      seasonTicketPurchasedAt: hasSeasonTicket ? seasonTicket.created_at : null,
    });
  } catch (err: any) {
