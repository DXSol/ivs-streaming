...
import streamProxyRoutes from './routes/stream-proxy';
...
async function main() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    app.use(
      cors({
        origin: ['http://localhost:8100', 'http://localhost:4200','http://localhost','capacitor://localhost'],
        credentials: true,
  ...
  app.use('/api/stream-proxy', streamProxyRoutes);
